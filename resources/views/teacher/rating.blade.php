...
@section('content')
                ...
                <form action="{{ route('teacher.rating.store') }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="score" max="{{ $task->score }}" min="0"
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('teacher.rating.edit') }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label>Maksimal {{ $task->score }} ball qo'yish mumkin</label>
                            <input id="score" type="number" name="score" max="{{ $task->score }}" min="0"
                                class="form-control" required>
                        </div>
                        @error('score')
                            <div class="text-danger">
    ...
    <div class="app-content-header">
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-sm-6">
                    <a href="{{ route('teacher.subject.tasks', ['course' => $task->course]) }}"
                                        ...
                                        <tr class="align-middle">
                                            ...
                                            @if ($student['submitted'])
                                                <td class="text-center"><a
                                                        href="{{ asset('storage/' . $student['file']) }}" download><i
                                                            class="bi bi-download"></i></a></td>

                                                @if ($student['rating_id'] && ($student['score'] == 0 || $student['score'] > 0))
                                                    <td class="text-center">{{ $student['score'] }}</td>
                                                    <td class="text-center">
                                                        <button id="editRatingBtn" data-bs-toggle="modal"
                                                            data-bs-target="#updateRating"
                                                            data-id="{{ $student['rating_id'] }}"
                                                            data-score="{{ $student['score'] }}"
                                                            data-comment="{{ $student['rating_comment'] }}"
                                                            class="btn btn-primary">O'zgartirish</button>
                                                    </td>
                                                @else
                                                    <td class="text-center"></td>
                                                    <td class="text-center">
                                                        <button id="ratingBtn" data-bs-toggle="modal"
                                                            data-bs-target="#createRating"
                                                            data-id="{{ $student['answer_id'] }}"
